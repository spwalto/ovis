...
**strgp_add**
   | plugin=store_sos name=<policy_name> schema=<schema> container=<container> [decomposition=<DECOMP_CONFIG_FILE_JSON>]
...
**sos_part_delete**
   | -C <path> <name>
   | Delete a partition in a container. The partition must be in the OFFLINE state to be deleted.
